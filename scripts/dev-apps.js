...
async function startDevelopment(selectedApps) {
  ...
  console.log(`${colors.green}• ${[...selectedApps].join("\n• ")}${colors.reset}\n`)
  ...
  try {
    ...
    execSync(
      `PORT=${backendPort} NEXT_PORT=${frontendPort} dotenv -e .env.local -- concurrently -c "blue.bold,yellow.bold" -n "@paperclip/web:dev,@paperclip/server:dev" "pnpm --filter ./apps/frontend dev --port ${frontendPort}" "pnpm --filter ./apps/backend dev"`,
      { stdio: "inherit" }
    )
  } catch (error) {
